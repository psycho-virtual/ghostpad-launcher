...
export const LocalNetworkIndicator: React.FC = () => {
  ...
  // Don't show if we're already on local network or local isn't available
  if (!isLocalAvailable || (isSupported && chain?.id === 31337)) return null;
  ...
  return (
    <div className="fixed top-20 right-4 z-40 bg-ghost-dark border border-yellow-500/30 p-2 rounded-md text-xs">
      <div className="flex items-center gap-2">
        ...
        <span>Local network running {!isSupported ? "but wallet connection unavailable" : "but not connected"}</span>
      </div>
      {isSupported && (
        <button 
          onClick={async () => {
